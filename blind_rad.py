'''
function name: blind_rad.py - complete

blind_rad.py is considered as blind where the post processing agent can pick the resources that previous agents have already picked.

blind_rad start from each agent, pick any R value that is avaialble in action.Px.Rx, add it to the sum_r. 
'''
import random
import copy
import pandas as pd

def blind_rad(X_test):
    X_testcopy = copy.deepcopy(X_test)
    #combined_data = pd.DataFrame()
    #X_testcopy.to_csv('./'+'blindrad_init.csv')
    ...
    #X_testcopy.to_csv('./'+'blindrad_final.csv')
    #combined_data = pd.concat([combined_data, X_testcopy], ignore_index=False)
    #combined_data.to_csv('./'+'blindrad_combinedall.csv')
    return X_testcopy
